...
@testset "mass2 consistency" begin
    for mom in [mom_onshell, mom_zero]
        ...
        @test isapprox(
            LorentzVectorBase.invariant_mass(mom),
            sqrt(LorentzVectorBase.invariant_mass2(mom)),
        )
    end
end
...
@testset "momentum components" begin
    ...
    @test LorentzVectorBase.mass(mom_onshell) == m
    @test isapprox(LorentzVectorBase.boost_beta(mom_onshell), 
        LorentzVectorBase.spatial_magnitude(mom_onshell) / LorentzVectorBase.energy(mom_onshell))
...
@testset "transverse coordinates values" begin
    ...
    @test isapprox(LorentzVectorBase.mt2(mom_onshell), LorentzVectorBase.energy(mom_onshell)^2 - z^2)
    @test isapprox(LorentzVectorBase.mt(mom_onshell), sqrt(LorentzVectorBase.mt2(mom_onshell)))
    @test isapprox(LorentzVectorBase.rapidity(mom_onshell), 0.5 * log((LorentzVectorBase.energy(mom_onshell) + z) / (LorentzVectorBase.energy(mom_onshell) - z)))
